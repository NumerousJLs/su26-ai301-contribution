...
**Branch:** `fix/project-picker-search-all-projects` (off `dev` @ `c8487bac54`)  
...
The one real friction: `bunx tsc --noEmit -p tsconfig.json` in `packages/app` reports 13 errors in the two files I touched even on a clean checkout of `dev`. They are all `TS7006: implicitly has an 'any' type` cascading from `Cannot find module '@opencode-ai/client/promise'` — an unbuilt workspace dependency, not real type errors. I confirmed this by stashing my changes and re-running: 13 errors before, 13 after. Worth documenting, because on first run it looks like the change broke the build.
...
Added to `packages/app/src/components/directory-picker-domain.test.ts` (commit `1420ec08c2`):
...
- `bunx prettier --check` on all three changed files → "All matched files use Prettier code style!"
- Typecheck against a stashed baseline: 13 pre-existing errors in these files on clean `dev`, 13 after the change. None introduced. (Details in Environment Setup above.)
...
I did not run the Desktop app end to end for this. The behavior depends on having six or more real projects with session history, and the code path is short enough to verify by reading. That is a limitation of my verification, and I would rather state it than imply coverage I don't have.
...
**Commit 1 — `b6f9e98003`:** `fix(app): search every known project in the open project dialog`
...
**Commit 2 — `1420ec08c2`:** `test(app): pin regression for recent project search candidates`
...
The branch is complete and verified locally — both commits are in place, tests pass, and the four pre-submission gates were re-checked. The PR has not been pushed yet because I review the diff and PR description myself before anything goes to the upstream repo. This section will be updated with the link, submission date, and any maintainer feedback once it is opened.

**Prepared PR description:**

> Closes #39142
>
> Typing a project name into the Open Project dialog now finds that project even when it is not one of the five most recent.
>
> Before this change, `recentProjects()` in `packages/app/src/components/dialog-select-directory.tsx` sorted every known project by last session activity and then called `.slice(0, 5)` while building its rows. Those rows are what the dialog hands to `List`, and `List` runs its fuzzy filter over the array it receives. Slicing inside the memo meant only five projects were ever candidates for that filter, so searching for a sixth project returned "No folders found" even though the project was open and visible in the sidebar. Entering the full absolute path still worked, because that goes through the directory search rather than the recent-project list.
>
> This PR moves the cap out of the memo and into the point where the query is known. `recentProjects()` now returns rows for every known project, and `visibleRecentProjects()` applies the five-item limit only when the search box is empty. The idle dialog looks exactly as it did before; once the user types, the whole set is searchable. The limit helper lives in `directory-picker-domain.ts` next to `currentPickerSuggestions()`, which is the existing pattern in this file family for query-dependent logic that needs a test.
>
> `bun test src/components/directory-picker-domain.test.ts` in `packages/app` passes 21 tests. The new case fails against the old behavior, since the previous code returned five rows regardless of the query.
...
- [x] No new typecheck errors (verified against a stashed baseline)
